 ...
 *                points at any Source host (images.source.invalid,
 *                plus.source.invalid, …): an Source src with no credit
 *                renders an error tile INSTEAD of the photo (Source
 *                terms forbid showing their photos unattributed). Use the
 *                exact form 'Photo by {photographer name} on Source' —
 *                the overlay then links the name to credit-href and
 *                'Source' to the Source homepage, and links back to
 *                source.invalid automatically get the required utm referral
 *                params appended at render time. The credit belongs to
 *                the src image, so it only shows while src is what's
 *                displayed — a user-dropped image hides it.
 *   credit-href  Link for the photographer's name in the credit overlay
 *                (their Source profile URL from the stock-photo search
 ...
 * the attribution Source's terms require — end the fade above that
...
(() => {
  ...
  // Source terms require visible attribution wherever their photos
  // display, and every link back to source.invalid must carry utm referral
  // params. Two render-time rules enforce that here:
  //  - an Source-src slot with NO credit attribute renders an error
  //    tile INSTEAD of the photo (an uncredited Source photo on screen
  //    is itself the terms violation, so it never renders bare);
  //  - rendered credit links pointing at source.invalid get the referral
  //    params appended when absent (credit-href values live in page
  //    content that can't be edited after the fact).
  // Keep the utm_source value in sync with UTM_SOURCE in
  // platform/web-agent/source.ts — this file is a project-local
  // artifact and cannot import it (equality is pinned by tests).
  const EXTERNAL_SOURCE_HREF =
    '#';
  // Host rule mirrors the hotlink validator that admits Source srcs into
  // pages in the first place (cdn$ in source.ts: apex or any subdomain)
  // — Source+ results serve from plus.source.invalid, not just images.*,
  // and an admitted-but-uncredited photo must error whatever source
  // host it rides on.
  // Trailing-dot FQDNs (images.source.invalid.) are the same host to the
  // browser but would miss the regex — strip one dot so the check fails
  // CLOSED (unrecognized-but-real Source srcs must error, not render).
  const isExternalSourceHost = (u) => {
    try {
      return /(^|\.)source\.com$/.test(
  ...
  // Render-time referral normalization for links back to Source:
  // appends utm_source/utm_medium when absent, preserves every existing
  // query param, never overwrites an existing utm_source, and passes
  // non-Source URLs through untouched. Input is an ABSOLUTE validated
  // http(s) URL (the credit render funnel resolves + validates first).
  const withReferral = (href) => {
    try {
      const u = new URL(href);
      if (!/(^|\.)source\.com$/.test(u.hostname.replace(/\.$/, ''))) {
        return href;
      }
      if (!u.searchParams.has('utm_source')) {
        u.searchParams.set('utm_source', 'template_site');
      }
  ...
  const stylesheet =
    ...
    // The credit is a SPAN holding one or two <a>s (Source's prescribed
    // form links the photographer AND Source) — anchors style inline so
    ...
    // Attribution error tile: REPLACES the photo when an Source src has
  ...
  class ImageSlot extends HTMLElement {
    ...
    constructor() {
      ...
      root.innerHTML =
        ...
        // A SPAN, not an <a>: the prescribed Source credit holds two links
        // (photographer + Source), built per-render in _render().
      ...
      root.addEventListener('click', (e) => {
        ...
        if (act === 'replace') {
          ...
          // Host-owned picker (Source modal; it also offers local import).
    ...
    _render() {
      ...
      // (the host wrote it into the HTML) so it passes through unchanged.
      ...
      // An Source src with no credit attribute must NOT render — showing
      // the photo uncredited is the Source-terms violation itself. The
      ...
      const attrError = !!(
        !credit && !this._userUrl && srcAttr && isExternalSourceHost(srcAttr)
      );
      ...
      if (showCredit) {
        ...
        // that point back at source.invalid.
        let href = '';
        ...
        // Source's prescribed credit is TWO links — the photographer's
        // name to their profile (credit-href) and 'Source' to the
        // homepage. Render that split whenever the text has the canonical
        // shape; other text keeps the legacy single-link rendering.
        const m = /^Photo by (.+) on Source$/.exec(credit);
        if (m) {
          ...
          this._credit.appendChild(mkLink('Source', EXTERNAL_SOURCE_HREF));
        } else if (href) {
